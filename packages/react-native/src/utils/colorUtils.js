...
export const getColorByProp = (color, theme, variant, neutralNumber) => {
  switch (color) {
    case "primary":
    ...
    case "warning":
      return theme?.color?.warning?.[variant || "main"] || Color.Warnning.main;
    case "error":
    ...
    case "neutral":
      return (
        theme?.color?.info?.[variant][neutralNumber || 300] ||
        Color.Neutral[500]
      );
    default:
      return theme?.color?.primary?.[variant || "main"] || Color.Navy.main;
